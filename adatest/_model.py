...
class Model():
    ...
    def __init__(self, model, output_names=None,url="",api_key="",query="", **kwargs):
        ...
        if self.__class__ is Model:
            ...
            elif model == "SafetyNetAPI":
                self.__class__ = SafetyNetAPI
                SafetyNetAPI.__init__(self, model,url,api_key,query, **kwargs)                
            else:
...
class SafetyNetAPI(Model):
    def __init__(self,model, url, api_key,query):
        header = {'Content-Type':'application/json', 'Authorization':('Bearer '+ api_key)}
        self._inner_model = APICaller(url,header, bottype ="safetynet")
        self.output_names = None
        self.query = query
    ...
    def __call__(self,sequences,candidate_labels, *args, **kwargs):
        ...
        for seq in sequences:
            if not bool(seq and not seq.isspace()):
                ...
                seq = "[EMPTYSEQ]"          
            try:
                inner_out = self._inner_model(seq,candidate_labels,hypothesis_template=self.query)
            except:
                #patch the score when the empty string check miss
                seq = "[FAULTYSEQ]" 
                inner_out = [[x,-1] for x in candidate_labels]
    ...
    def getscoreandlabel(self,x):
        print("x",x)
        labels = x["entities"]
        prob = x["max_entity_probabilities"]
        print("prob",prob)
        prob["acceptable"] = 1 - prob["unacceptable"]
        scores = [prob[sc] for sc in labels]
        if scores == []:
            scores = [1]
            labels = ["unclass"]
...
class APICaller():
    ...
    def __call__(self,seq,candidate_labels=[],hypothesis_template="",multi_class=False, th=0):
        if self.bottype == "entailment": # extend here for future endpoint integration
            ...
            data = {"utterance": seq}
        elif self.bottype == "safetynet": # extend here for future endpoint integration
            data = {"generation": seq, "query":hypothesis_template}
        #try:
        print(data)
